...
public class ARCameraCompassAlignment : MonoBehaviour
{
    ...
    private float CompassAccuracy
    {
        get
        {
            float compassAccuracy = this.locationProvider.CurrentLocation.Accuracy;
    ...
    {
        float inverseRotation = (orientation - this.arController.arCamera.transform.localEulerAngles.y) * -1;
        this.arController.rotation = Quaternion.Euler(new Vector3(0, inverseRotation, 0));
    }
